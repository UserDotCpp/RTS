...
export(Resources) var reso #this only goes if whe need to instance the resource manualy
...
onready var detect_area = $aCollition
export var sight_range = 10.0


func show_sprite(num):
	if(num != -1 and num != 4):
		reso = num
		$sprites.get_child(num).show()
		working_status = true
...
func check_idle(): #call_idle
	...
	for unit in detect_area.get_overlapping_bodies():
		if !is_instance_valid(unit) or "team" in unit:#if it isnt deleted or if it doesnt have a "team"
			continue
		if !unit.get_parent().idle_status:
			return
